...
int main(void){
    ...
    if(POPSIZE % 2 != 0 && POPSIZE > 0){
        fprintf(stderr, "Population size is not a multiple of two! or population size <= 0\n");
        exit(1);
    }

    if(ELITISM % 2 != 0 && ELITISM >= 0){
        fprintf(stderr, "Elitism is not a multiple of two or elitism < 0!\n");
        exit(1);
    }
