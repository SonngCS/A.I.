...
void CrossOver(Population * pop, Population * newPop, int * idx){
    ...
    for(int i = ELITISM; i < POPSIZE; i+=2){
        ...
        if(mate > 80){  //80% of mating chance, or else the parents are also copies to the next generation
...
void Mutation(Population * pop, int i, int j){ //Simple mutation function
   ...
   // srand(time(NULL));
    ...
    if(mutation < 0.0369){ //If random number between 0 and 1 is less than 0.04, the gene will be inversed (mutation will be executed)
